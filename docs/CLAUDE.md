...
| `cae.ts` | CAE: material cards, panel weight, sag screening, orthotropic Mindlin plate FEM + ASSEMBLY flat-shell solver (membrane+bending, 6 DOF/node, penalty joints rigid/semi-rigid/hinged, floor grounding, Jacobi-PCG — 348 iters @ 58k DOF on the workbench). tests/cae_check.ts has SIX validation cases — keep all passing. Per-panel INTERACTIVE CAE was removed (user: useless); the ONLY CAE surface is the Analysis sidebar MODE (segmented Cut planning / Analysis switch under the brand header, green dot when solved; joints list + presets + patch loads). Structure table + Assembly PDF page gate on an assembly solve |
